...
document.addEventListener('DOMContentLoaded', () => {
  filterSearches(); // Initially display all searches
});

function searchAndStore() {
  const searchValue = document.getElementById('searchInput').value.trim();
  if (!searchValue) return; // Avoid storing empty strings
  ...
  filterSearches(); // Update list based on current input
  document.getElementById('searchInput').value = ''; // Optional: clear the input after search
}

function filterSearches() {
  const input = document.getElementById('searchInput').value.toLowerCase();
  if (input.length === 0) {
    displaySearches([]); // Clear the list if input is empty
    return;
  }
  const searches = JSON.parse(localStorage.getItem('searches')) || [];
  const filteredSearches = searches.filter(search => search.toLowerCase().startsWith(input));
  displaySearches(filteredSearches);
}
...
document.getElementById('searchInput').addEventListener('input', () => {
  if (document.getElementById('searchInput').value.trim() === '') {
    displaySearches([]); // Clear and hide the list when input is cleared
  } else {
    filterSearches(); // Otherwise, filter and display searches
  }
});